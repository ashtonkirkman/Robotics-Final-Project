...
# %% Define the robot
d1 = 1.2            #centimeters / 10 or decimeters
a1 = 0.15
a2 = 1.2
d4 = 1.4
tip_thickness = 0.15
...
joints = ['r', 'r', 'r', 'r']
q_set1 = [0, 0, 0, 0]
q_set2 = [-0.17, 18.51, -35.845, 0.422]

R_tip = tf.roty(-np.pi/2) @ tf.rotx(np.pi/2)
T_tip = tf.se3(R_tip)
arm = kin.SerialArm(dh_params, jt=joints, tip=T_tip)
golf_ball_position = np.array([[1.5], [0], [0.2]])
hole_position = np.array([[2.5], [1.5], [0.2]])
...
adjusted_solution_degrees = np.round((adjusted_solution_degrees + 180) % 360 - 180)
